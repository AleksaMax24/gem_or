...
bool operator==(color const& rgb1, color const& rgb2) { //оператор сравнения цветов
	return (rgb1.r == rgb2.r && rgb1.g == rgb2.g && rgb1.b == rgb2.b);
}

GameField::GameField() { // инициализация поля
...
void GameField::drawField() const {
	...
	glTranslatef(-getW() / 2.0f, -getH() / 2.0f, 0); //функции для выравнивания поля
...
void GameField::swapGems() { //перемещение гемсов
	int dx = 1, dy = 1;
	for (int x = 0; x < getW(); x++) {
		for (int y = 0; y < getH(); y++) { 
			if (getCell(x, y)->is[SELECT]) { 
...
void GameField::SmashOut(xy const& position) { //функция "уничтожения" гемса
...
bool comparator(xy const& point1, xy const& point2) { // компаратор для сортировки вектора
